    ...
    <div class="card-body">
        ...
        <form id="invoiceForm" action="{{ route('finance.invoice.update', $invoice->id) }}" method="POST">
            ...
            <div class="form-check mb-3">
                <input type="hidden" name="is_same_with_po" value="0">
                <input class="form-check-input" type="checkbox" name="is_same_with_po" id="sameWithPo" value="1" {{ $isSameWithPo ? 'checked' : '' }}>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function loadInvoiceItems() {
            ...
            invoiceItems.forEach((item) => {
                ...
                // pastikan kita tetap render item lama, walaupun perizinan kosong
                const perizinanId = item.perizinan_id ?? '';
                const perizinanLainnya = item.perizinan_lainnya ?? '';
        ...
        `);
                ...
                // set value select sesuai input user lama
                const select = itemsContainer.querySelectorAll('.perizinan-select')[index];
                if (perizinanId) {
                    select.value = 'id:' + perizinanId;
                } else if (perizinanLainnya) {
                    const option = document.createElement("option");
                    option.value = perizinanLainnya;
                    option.text = perizinanLainnya;
                    option.selected = true;
                    select.appendChild(option);
                    select.value = perizinanLainnya;
                }

                index++;
            });

            // kalau tidak ada item sama sekali, tambahkan row manual
            if (index === 0) {
                addManualRow();
            }
